        ...
        try:
            ...
            for i, linha in linhas:
                ...
                try:
                    ...
                    # *** MARCAR COMO CONCLUÍDO ANTES DE PROCESSAR ***
                    try:
                        ...
                        log_interface(f"✓ Linha {i} marcada como concluída no Google Sheets.")
                    except Exception as err_up:
                        log_interface(f"⚠️ Não foi possível marcar linha {i} como concluída: {err_up}")
                        log_interface(f"⏭️ Pulando linha {i} e tentará novamente no próximo ciclo.")
                        continue  # Pula esta linha e tenta novamente depois
