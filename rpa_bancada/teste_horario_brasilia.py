...
def enviar_teste():
    ...
    try:
        ...
        linha_teste = [
            "Teste RPA Bancada",             # Coluna A: Descrição
        ...
        try:
            ...
            print(f"⚠️ Erro ao limpar planilha: {e}")
        ...
        # Preparar dados completos (cabeçalhos + linha de teste)
        dados_completos = [headers, linha_teste]

        # Enviar dados
        print("📤 Enviando dados para planilha...")
        result = service.spreadsheets().values().update(
            spreadsheetId=SPREADSHEET_ID,
            range=f"{sheet_name}!A1:G2",
            valueInputOption="RAW",
            body={"values": dados_completos}
        ).execute()
...
if __name__ == "__main__":
    print()
    print("🕐 TESTE DE HORÁRIO DE BRASÍLIA - RPA BANCADA")
    print()
