# -*- coding: utf-8 -*-
"""
Módulo de integração com Google Sheets para RPA Ciclo
Registra histórico de execução dos ciclos
"""
...
# BASE_DIR: Pasta onde o executável está (para token.json, processados.json, etc)
# Quando frozen (compilado): pasta do .exe
# Quando não compilado: pasta do script
from pathlib import Path
BASE_DIR = Path(__file__).parent.resolve() if not getattr(sys, 'frozen', False) else Path(sys.executable).parent

# Configurações do Google Sheets
SCOPES = ["https://www.googleapis.com/auth/spreadsheets"]
SPREADSHEET_ID = "14yUMc12iCQxqVzGTBvY6g9bIFfMhaQZ26ydJk_4ZeDk"  # Mesma planilha do RPA_Oracle
...
def authenticate_google():
    ...
    """
    # token.json fica no diretório atual (igual RPA Oracle antigo)
    ...
    # DEBUG: Mostra onde está procurando o token
    print(f"[AUTH] Procurando token em: {token_path}")
    print(f"[AUTH] Token existe? {os.path.exists(token_path)}")
    print(f"[AUTH] CredenciaisOracle em: {creds_path}")
    print(f"[AUTH] Credenciais existe? {os.path.exists(creds_path)}")
...
def registrar_ciclo(
    ...
    try:
        ...
        print(f"✅ Ciclo #{ciclo_numero} registrado no Google Sheets")
        return True
...
def atualizar_ciclo(ciclo_numero, campo, valor):
    ...
    try:
        ...
        print(f"✅ Ciclo #{ciclo_numero} atualizado: {campo} = {valor}")
        return True
...
if __name__ == "__main__":
    print("Testando módulo Google Sheets Ciclo...")
