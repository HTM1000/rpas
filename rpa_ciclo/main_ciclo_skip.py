...
class CacheLocal:
    """
    Gerencia cache local de itens processados para evitar duplicações.
    Usa "Data Hora Sep." como ID único ao invés de número da linha.
    """
    def __init__(self, arquivo="processados.json"):
        # Salva na pasta rpa_oracle (compartilhado com RPA_Oracle.py)
        rpa_oracle_dir = BASE_DIR.parent / "rpa_oracle"
        rpa_oracle_dir.mkdir(exist_ok=True)
    ...
    def _salvar(self):
        ...
        try:
            ...
            gui_log(f"⚠️ Erro ao salvar cache: {e}")

    def ja_processado(self, data_hora_sep):
        """Verifica se item já foi processado usando Data Hora Sep."""
        with self.lock:
    ...
    def adicionar(self, data_hora_sep, linha_atual, item, quantidade, referencia, status="pendente"):
        """
        Adiciona item ao cache após Ctrl+S no Oracle.
        Status pode ser: 'pendente' (salvo no Oracle mas não no Sheets) ou 'concluido'
        """
        with self.lock:
    ...
    def marcar_concluido(self, data_hora_sep):
        """Marca item como concluído quando Sheets for atualizado com sucesso"""
        with self.lock:
            if data_hora_sep in self.dados:
                self.dados[data_hora_sep]["status"] = "concluido"
                self.dados[data_hora_sep]["data_conclusao"] = datetime.now().isoformat()
                self._salvar()

    def get_pendentes(self):
        """Retorna lista de itens pendentes de sincronização com Sheets"""
        with self.lock:
            return {k: v for k, v in self.dados.items() if v.get("status") == "pendente"}
...
def etapa_05_executar_rpa_oracle(config):
    ...
    try:
        ...
        # Versão SKIP - tenta uma única vez, se não houver dados pula para a bancada
        itens_processados = 0
        ...
        if not valores:
            gui_log("⚠️ Nenhuma linha encontrada no Google Sheets - PULANDO para Bancada")
            return True
        ...
        if not linhas_processar:
            gui_log("⏳ Nenhuma linha para processar - PULANDO para Bancada")
            return True
        ...
        for i, linha in linhas_processar:
            ...
            # Adicionar ao cache APÓS Ctrl+S (status pendente)
            cache.adicionar(
            ...
            try:
                ...
                # Item fica como "pendente" no cache para retry posterior
            itens_processados += 1
            time.sleep(0.5)
...
def etapa_06_navegacao_pos_oracle(config):
    ...
    # Clicar no campo janela (338, 172)
    ...
    clicar_coordenada(784, 452, duplo=True, descricao="Duplo clique para abrir Bancada")
